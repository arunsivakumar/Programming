...
final class ChangePasswordViewControllerTests: XCTestCase {
    ...
    func test_tappingCancel_withFocusOnOldPassword_shouldResignThatFocus() {
        ...
        XCTAssertEqual(sut.oldPasswordTextField?.isFirstResponder, true,
                "precondition")

        tap(sut.cancelBarButton)

        XCTAssertEqual(sut.oldPasswordTextField?.isFirstResponder, false)
    }

    func test_tappingCancel_withFocusOnNewPassword_shouldResignThatFocus() {
        putFocusOn(.newPassword)
        XCTAssertEqual(sut.newPasswordTextField?.isFirstResponder, true, "precondition")
    ...
    func test_tappingCancel_withFocusOnConfirmPassword_shouldResignThatFocus() {
        ...
        XCTAssertEqual(sut.confirmPasswordTextField?.isFirstResponder, true, "precondition")

        tap(sut.cancelBarButton)

        XCTAssertEqual(sut.confirmPasswordTextField?.isFirstResponder, false)
    }
    ...
    func test_tappingSubmit_withValidFields_shouldDisableCancelBarButton() {
        setUpValidPasswordEntries()
        XCTAssertEqual(sut.cancelBarButton.isEnabled, true, "precondition")

        tap(sut.submitButton)

        XCTAssertEqual(sut.cancelBarButton.isEnabled, false)
    }
    ...
    func test_tappingOKInFailureAlert_shouldEnableCancelBarButton() throws {
        showPasswordChangeFailureAlert()
        XCTAssertEqual(sut.cancelBarButton.isEnabled, false, "precondition")

        try alertVerifier.executeAction(forButton: "OK")

        XCTAssertEqual(sut.cancelBarButton.isEnabled, true)
    }
    ...
    func test_hittingReturnFromNewPassword_shouldPutFocusOnConfirmPassword() {
        ...
        XCTAssertEqual(sut.confirmPasswordTextField?.isFirstResponder, true)
    }
