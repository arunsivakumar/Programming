...
class TweetTableViewController: UITableViewController, UITextFieldDelegate
{
    ...
    var managedObjectContext:NSManagedObjectContext? = (UIApplication.sharedApplication().delegate as? AppDelegate)?.managedObjectContext //11 store data
                ...
                dispatch_async(dispatch_get_main_queue()) {
                    if request == weakSelf?.lastTwitterRequest {
                        if !newTweets.isEmpty {
                            ...
                            weakSelf?.updateDatabase(newTweets) //11
                        }
                    }
    ...
    private func updateDatabase(newTweets:[Twitter.Tweet]){ //11
